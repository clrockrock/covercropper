...
const source = readFileSync(join(process.cwd(), 'packages/angular/src/index.ts'), 'utf8')
...
describe('@covercropper/angular wrapper forwarding contract', () => {
  it('uses the Web Component as the single implementation and marks the custom element schema valid', () => {
    ...
    expect(source).toContain('CUSTOM_ELEMENTS_SCHEMA')
    expect(source).toContain('schemas: [CUSTOM_ELEMENTS_SCHEMA]')
    expect(source).toContain('<cover-cropper #cropper></cover-cropper>')
  })

  it('forwards inputs, outputs, two-way value updates and public methods', () => {
    ...
    expect(source).toContain('valueChange.emit')
